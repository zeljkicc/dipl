...
//sb.append(StringUtils.newStringUtf8(Base64.encodeBase64(cursor.getBlob(index), false)));
//String contourChart = sb.toString();

      //return StringUtils.newStringUtf8(Base64.encodeBase64(cursor.getBlob(index), false));


     // return Base64.encodeToString(cursor.getBlob(index), Base64.NO_WRAP);

      //return new String(Base64.encode(cursor.getBlob(index), Base64.DEFAULT));

//return contourChart;
    // return Base64.encodeToString(cursor.getBlob(index), Base64.DEFAULT);

    // return Base64.encodeBase64URLSafeString(cursor.getBlob(index));
       //return Base64.encodeToString(new String(cursor.getBlob(index)), Base64.DEFAULT);
      //return new String(Base64.encode(cursor.getBlob(index), Base64.DEFAULT));
